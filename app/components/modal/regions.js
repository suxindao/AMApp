...
export default class Regions extends Component {
    ...
    getNextCityData(item, page) {
        if (page == 0) {
            ...
            this.post(
                ...
                response => {
                    // 直辖市只显示两级
                    // if(response[0].region_id.indexOf('99')>0){
                    //   let req = {
                    //     _AT: global.UserInfo.token,
    ...
    subInfo() {
        this.hide()
        this.props.cityCallback(this.ChooseData)
    }

    previseClick(page) {
        this.ChooseData.splice(page - 1, 1);
        if (x > 0) {
            x -= WIDTH
        }
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ViewControl leftTitle='上一步' title='请选择' data={this.state.classB} page={1}
                                                 ...
                                                 itemTouch={this.clickItem}
                                    />
                                    <ViewControl leftTitle='上一步' title='请选择' data={this.state.classC} page={2}
                                                 previseTouch={this.previseClick} closeTouch={this.hideModal}
                                                 itemTouch={this.clickItem}
                                    />
                                </ScrollView>
        ...
        return (
            <View style={{width: WIDTH, height: Height, flexDirection: 'column'}}>
                <View style={{
                    ...
                    <TouchableOpacity style={{paddingLeft: distances.leftMargin}}
                                      onPress={this.previseClick}
                    >
                    ...
                    <TouchableOpacity style={{paddingRight: distances.leftMargin}}
