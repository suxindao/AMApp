...
import {loadMoreData, reloadData} from '../../../../redux/modules/contract/otherListReducer'
...
const mapStateToProps = state => ({
  list_loading: state.contractOther.list_loading,
  list_refreshing: state.contractOther.list_refresh,
  list_error: state.contractOther.list_error
})
...
class Page extends Component{
  ...
  renderListRow(rowData, sectionID, rowID){
    return <ListItem rowData={rowData} itemType={this.props.contractType}/>
  }
  ...
  async _getMore(){
    try{
      let body = handleContractsBody(this.props.searchContent, this.props.contractType, this.data.next_key)
  ...
  async _getReload(){
    try{
      let body = handleContractsBody(this.props.searchContent, this.props.contractType, '')
      let ret = await fetchContractOtherDataWithoutCache(body, '')()
      if(typeof this.data.ended === 'boolean'){
        this.data.ended = ret.ended 
      } 
